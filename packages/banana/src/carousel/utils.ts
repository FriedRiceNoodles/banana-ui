...
export function getCoordinates(e: Event) {
  ...
  const x = _isMouseEvent ? (e as MouseEvent).clientX : (e as TouchEvent).touches[0].clientX;
  const y = _isMouseEvent ? (e as MouseEvent).clientY : (e as TouchEvent).touches[0].clientY;
  return [x, y];
}
